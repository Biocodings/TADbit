...
def run(opts):
    ...
    if not opts.resume:
        ...
        hist_path = path.join(opts.workdir, '03_filtered_reads',
                              'histogram_fragment_sizes_%s.%s' % (param_hash, opts.fig_format))
    ...
    for cov_reso in opts.cov_reso:
        outplot = path.join(opts.workdir, '03_filtered_reads',
                            'genomic_coverage_%s_%s.%s' % (nice(cov_reso),
                                                              param_hash,
                                                              opts.fig_format))
        outdata = path.join(opts.workdir, '03_filtered_reads',
                            'genomic_coverage_%s_%s.tsv' % (nice(cov_reso),
                                                               param_hash))
